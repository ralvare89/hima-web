...
const infoCards = [
  ...
  {
    key: 'whatsapp',
    valKey: 'whatsapp_value',
    href: (val) => `https://wa.me/${val.replace(/[\s+\-]/g, '')}`,
    target: '_blank',
    icon: (
    ...
    <div className="pt-28">
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-5 gap-10 lg:items-stretch">
          {/* Form — 3 cols */}
          <AnimatedSection className="lg:col-span-3 flex flex-col" direction="right">
            <div className="bg-hima-card border border-white/5 rounded-lg p-8 flex flex-col flex-1">
              {sent ? (
                <motion.div
          ...
          <div className="lg:col-span-2 flex flex-col">
            <StaggerContainer className="flex flex-col gap-4 flex-1 justify-between">
              {infoCards.map(({ key, valKey, href, target, icon }) => {
                  ...
                  <StaggerItem key={key}>
                    <div className="bg-hima-card border border-white/5 rounded-lg p-5 flex items-start gap-4 hover:border-hima-green/25 transition-colors duration-300">
                      ...
                      <div>
                        ...
                        {link ? (
                          <a href={link} target={target} rel={target === '_blank' ? 'noopener noreferrer' : undefined} className="text-hima-white font-medium text-sm hover:text-hima-green transition-colors duration-200">
                            {val}
                          </a>
                        ) : (
